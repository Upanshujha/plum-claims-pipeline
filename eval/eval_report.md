...
Generated: 2026-04-26T07:21:26.251666Z
...
- Expected: `{'decision': None, 'system_must': ['Stop before making any claim decision', 'Tell the member specifically what document type was uploaded and what is needed instead', 'Not return a generic error â€” the message must name the uploaded document type and the required document type']}`
...
- Expected: `{'decision': 'APPROVED', 'approved_amount': 1350, 'notes': '10% co-pay applied on consultation category (â‚¹150 deducted)', 'confidence_score': 'above 0.85'}`
...
- User message: _Your Consultation Claim for ₹3,000 is Not Eligible Yet This claim falls under a 90-day waiting period for type 2 diabetes mellitus treatment, as per our policy. You will be eligible for diabetes-related claims from 30-Nov-2024, so please wait until then to re-submit your claim._
- Trace: `intake:PASS → classifier:PASS → sufficiency:PASS → parser:PASS → quality:PASS → consistency:PASS → rules_engine:PASS → fraud:PASS → synthesizer:PASS → rejection_explainer:PASS`
- Match: **True** — decision and amount match expected
...
- User message: _Pre-authorization is required for your diagnostic claim Your claim of ₹15,000 for the diagnostic test on 2024-11-02 was rejected due to missing pre-authorization Please request pre-auth via the member portal before your next test, attach the approval reference to your claim, and submit again_
- Trace: `intake:PASS → classifier:PASS → sufficiency:PASS → parser:PASS → quality:PASS → consistency:PASS → rules_engine:PASS → fraud:PASS → synthesizer:PASS → rejection_explainer:PASS`
- Match: **True** — decision and amount match expected
...
- User message: _Claim Exceeds Per-Claim Limit Your claimed amount of ₹7,500 exceeds the per-claim limit of ₹5,000 for this policy. Please split the claim into separate claims by provider or by date if the treatment genuinely cost more._
- Calc breakdown:
...
- Expected: `{'decision': 'APPROVED', 'approved_amount': 3240, 'notes': 'Network discount (20%) applied first on â‚¹4,500 = â‚¹3,600. Co-pay (10%) applied on â‚¹3,600 = â‚¹360 deducted. Final: â‚¹3,240.', 'system_must': ['Apply network discount before co-pay, not after', 'Show the breakdown of discount and co-pay in the decision output']}`
...
- User message: _Your consultation claim for ₹8,000 on 2024-10-18 is not covered. Your policy excludes coverage for obesity and weight loss programs, which is the condition you're claiming for. Please refer to the policy exclusions list for details and contact support if you have any questions._
- Trace: `intake:PASS → classifier:PASS → sufficiency:PASS → parser:PASS → quality:PASS → consistency:PASS → rules_engine:PASS → fraud:PASS → synthesizer:PASS → rejection_explainer:PASS`
- Match: **True** — decision and amount match expected
